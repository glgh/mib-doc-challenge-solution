...
- `cofire_probe.py` — the cascade structure audit (TODO Track 5, row 39): evaluates all 16 policy branch predicates independently per dev case against the full cache, prints the co-fire pair matrix, the per-cell EV audit vs the raw scoring matrix, and (`--truth`) the truth-branch → pipeline-branch migration matrix. Its ORDER list must stay in sync with `policy.adjudicate` (it asserts first-match equivalence). Run: `.venv/bin/python experiments/cofire_probe.py [--truth]`.
