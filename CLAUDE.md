...
**Env knobs**: `MIB_GEOM_SET`/`MIB_OPT_SET`/`MIB_OPT_BASE=raw|frames`/`MIB_LAYOUT_PASS=psm3|off` (grid module overrides, all stamped into `restore`; `MIB_LAYOUT_PASS` is the PSM-3 layout-pass re-read on a truncated field label, **default-on since 2026-07-27**, `off` reverts — TODO 6.7), `MIB_DEBUG_JSONL` (sidecar path), `MIB_OCR_OPTICAL` (optical variants, **on by default** since row 48; `off` for A/Bs), `MIB_WORKERS` (local wall-clock knob; default 4 = the contract, output byte-identical at any count). The per-case OCR bound (120 s) is a constant, not an env var. Three legacy knobs were deleted in the de-special-casing batch (2026-07-26, gates: replay IDENTICAL + verify_render 6/6): `MIB_PLAN=ladder` (the frozen pre-grid enumerator — reviving it or reverting the grid default now means git history, not an env flip), `MIB_SELECT=ev` (the hand-built evidence_score selector; conf-mass is the only selector), and `MIB_OCR_PASSES=dual` (the per-image PSM 3 second pass; the grid's layout-pass tier is the revival path).
