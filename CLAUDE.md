...
**Env knobs**: `MIB_PLAN=grid|ladder` (S2 enumeration plan; `grid` default since row 60 — the composition grid; `ladder` is the frozen pre-grid legacy for A/Bs), `MIB_GEOM_SET`/`MIB_OPT_SET`/`MIB_OPT_BASE=raw|frames`/`MIB_LAST_RESORT=off|psm3` (grid-only module overrides, all stamped into `restore`), `MIB_DEBUG_JSONL` (sidecar path), `MIB_OCR_PASSES=psm11|dual` (second PSM 3 pass per image; +0.87 dev but unshipped for cost, row 20 — the grid's last-resort tier is the gated revival path), `MIB_OCR_OPTICAL` (optical variants, **on by default** since row 48; `off` for A/Bs), `MIB_SELECT=conf|ev` (primary-read metric; `conf` default since row 43, `ev` for A/Bs until deleted), `MIB_WORKERS` (local wall-clock knob; default 4 = the contract, output byte-identical at any count). The per-case OCR bound (120 s) is a constant, not an env var.
