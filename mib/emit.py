...
# Validation here uses grammar's ^…$-anchored VALIDATE_* tier directly ("is
# *this* emitted value exactly well-formed?"), spelled at each call site so the
# find-vs-validate intent is never lost behind a local alias.
...
def validate(record, fallback_case_id=None):
    ...
    twice; validating and coercing it here is the last-line net. sponsor_id gets
    the same treatment: a malformed read is repaired if it can be, else
    defaulted — never emitted off-schema.
    """
    if not grammar.VALIDATE_CASE_ID.match(str(record.get("case_id", ""))):
    ...
    if not grammar.VALIDATE_SPONSOR.match(str(record.get("sponsor_id", ""))):
    ...
    if not grammar.VALIDATE_DATE.match(ad):
...
def _repair_case_id(value, fallback):
    ...
    if grammar.VALIDATE_CASE_ID.match(str(fallback or "")):
        return str(fallback)
    return "MIB-000000"
