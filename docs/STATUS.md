...
**Working tree: dev 122.44, CFA 0, 0 missing rows** (rows 30–34; row 34 voids red-strikethrough
value cells — a document crossing out its own value, e.g. a fee receipt's struck `unpaid`, is not
sourceable evidence — worth +0.20 over the keystone baseline, fixing two struck-fee false denials
with CFA 0; row 32 adds eroded-label registry
...
| 6 | Can the `illegible_biometrics` structural gap be read or argued? (~+0.8 dev ceiling) | **Mostly settled by the two-mode label mining (BACKGROUND §3, 2026-07-25, dev n=162).** Mode A (printed, 94/162): truth P=1.00 whenever the token is legible in any read; we emit 91 with 0 FPs; the 3 misses print the token on pages whose OCR-mangled header defeats `detect_doc_type`, so the flag scan's doc-type gate skips them — recoverable, and generalizes to a "printed-but-untyped-page" fix family. Mode B (condition-derived, 68/162): no printed token anywhere; every observable proxy tops out at P≤0.38 (B-13-seen-risk-line-unreadable 0.38 n=29; no-B-13+dead-page 0.30 n=113; `biometric_confidence` values interleave), because the generator knows whether the spec *included* a B-13 it then destroyed — the PDF shows the same picture either way. Condition-based emission is expected-negative on a weight-8 exact-set field; the emission argument is dead. **Mode B subdivides** (renders, user-verified): ≥5 of the 11 detected-B13 mode-B slips visibly print the token, OCR-mangled past the fuzzy matcher (`Bagitie bematics`) — the fix family is "flag line present, matcher/gate too strict" (3 doc-gate misses + label-anchored value recovery on `Observed flags:` lines); 2 more print damage markers (`[RISK PANEL MISSING]`) and are genuinely tokenless. The 57 no-B13-detected dead-page cases are unassessed page-by-page |
| 7 | Does Tesseract word-level confidence (TSV `conf` column) resolve the valid-vs-valid vote ties? | Capture TSV in `_tesseract` (same recognition pass, one dump regen), store per-line conf in `Read`, and A/B a conf-weighted vote offline on the ensemble cache. The earlier variant-merge probe found ties where generation order picks wrong in one direction each (025 date / 037 name) and only word conf wins both. **New evidence (2026-07-25, MIB-000990/252 ladder autopsy): TSV conf also disagrees with `evidence_score` on *page ranking* — on both pages every conf metric (mean conf, words≥70, conf mass) ranks the deskewed/deshredded variants above the tilted raw render that `evidence_score` chose. The tilted winner's margin came from a well-formed-but-WRONG value (`Case ID: MIB-000000`, misread of 000990, +1 via CASE_ID_RE) — shape-validity credits confident garbage; the engine's own conf does not. Per-line conf is the right unit for the flag scan; page-level conf mass rewards word count** |
