...
**State (2026-07-28): dev 128.15, CFA 0** (holdout last read 128.48 at dev 127.45; row 94's reason fallback moved 0 holdout decisions). The extraction/selection frontier is CLOSED with evidence: the read-creation ceiling is airtight across every OCR-config axis (PSM 3/4/6 + native hi-DPI + field-crop recovered ~0 of 35 dev misses — `experiments/ceiling_delta*.py`, 1.8 caveat), selection headroom is mostly already-worked (arb_oracle 63 → ~54 at HEAD, the fee bucket converted), and the merge tie-break "improvement" was falsified on diagnosis (5.11, `experiments/diagnose_tiebreak.py`). The milestone holdout read confirmed the gains generalized ~1:1 — no overfit. **Remaining real work is submission-hardening**: the Docker gate at grid cost (Track 4 / STATUS Q3), the current-rev validation drift re-check (5.6), and the memo (5.5). The open items below are mostly low-yield given the proven ceiling — pursue the repair items (Track 3) only for rank-1 evidence (findings / flags / stamps that flip a whole adjudication), not general field recovery.
...
**1.8 caveat — "extraction saturated" is CONFIG-RELATIVE.** The saturation verdict was stress-tested (2026-07-28) by EXPANDING the read set (PSM 3/4/6 + native hi-DPI + field-crop, `experiments/ceiling_delta*.py`): 30/35 dev misses stayed unreachable, so the ceiling is real — but it is measured against the current OCR config (PSM-11, 202 DPI, whole-page gating). Re-run the oracles if the OCR engine or render pipeline changes materially; do not read "0 reachable" as proof of absence without expanding the reads.
...
- **Docker gate at grid cost (STATUS Q3): STILL OPEN, mandatory before submission.** p50 ~19.1 s/case (post layout-pass, 6.7) vs the 120 s/case bound and ~11× contract headroom at skew; decides fits-vs-revert. Revert now means restoring the deleted ladder enumerator from git history (not a `DEFAULT_PLAN` flip).
