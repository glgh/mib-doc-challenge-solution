...
### 1.4 ☐ Conf-weighted variant-vote tie-break — **blocked on per-LINE conf (schema 4)**
Page-level weighting measured and REVERTED (row 43): 5 better / 8 worse — whole-read conf mass is uncorrelated with one field line's correctness, and it steered a tie into fabricating a revoked sponsor. A revoked-tie guard was likewise reverted (MIB-000130's `SPN-4040` is genuinely revoked; on a tie the revoked reading is the deny-safe direction). Ship only after the tsv-line-text capture (schema 4) lets a vote weigh the value's own line. Dev tie anchors still to be mined first (025/037 are HOLDOUT — motivate, never tune).
