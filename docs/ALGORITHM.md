...
| `local` | text-consent corrector (`imaging.realign_local`, row 51): border offsets from the merged reader (pair rows where the width test passes, one-line tracker fills snapped to the local pair level), overridden at seams that cut THROUGH text by cross-correlating the cut glyph halves — including "the text never moved, leave it", which rescues phantom-border pages the plain walk shears. Emitted only when its pixels differ from `bands`; whitespace seams keep border behaviour (cosmetic — the seam-through-text census showed only 28% of under-deshred pages have any text on a cut) |
