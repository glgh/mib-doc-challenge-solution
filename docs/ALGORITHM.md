...
> **Hazard (discharged for the grid, residual for the ladder path):** `evidence_score` calls `parse.key_for`, so a `KEY_MAP` edit could change which variant wins and silently invalidate the cache. Under the shipped `grid` plan the gate is the frozen `page_score` and selection is conf — S2 is a pure function of (image, config) again. The coupling persists only in the frozen `MIB_PLAN=ladder` legacy path; guard ladder-touching changes with `scripts/verify_render.py`.
