...
- **Shape:** ISO `YYYY-MM-DD`; `valid_value` rejects well-shaped-but-impossible dates (`2026-03-41`) and years outside **2020–2026** (rows 95–96, tightened from 2020–2030). The corpus tops out at 2026 (1,000 train labels: 948× 2026, 52× 2025, **zero 2027+**), so the upper bound now deliberately rides that distribution: `≥2027` is future-impossible garble. Lower bound stays wide — a past year is a plausible stale date, never repaired.
- **Sources ↓precedence:** intake, registry.
- **Decoys & traps:** systematic `6→8` year OCR error (`2028`→`2026`, 23/25); a **legible date value under a garbled key** (MIB-000614 registry `nal iste: 2028-04-29`, MIB-000457 `Antvel Dete 2026-03-29` — the key-anchored parse drops these; **value-anchored recovery now ships**, see Handling); missing/hidden-only arrival → NEEDS_REVIEW. The `1900-01-01` sentinel is emit-only (policy decides on the raw values first, so `missing_arrival` still fires on absence).
- **Handling:** `DATE_RE`; `grammar.coerce_arrival_date` snaps **any** future-impossible year (`≥2027 → 2026`, incl. 2-glyph garble `2928`/`2976` that used to be dropped) but never past ones (un-staling a real 2024 trades a 0-cost wrong denial for a −4 false approval). **Value-first recovery** (`fallbacks.arrival_date_fallback`, rows 95–96): when the merge leaves arrival_date empty because the label OCR'd to debris, scan the guarded lines for a date-shaped value directly (an ISO date is self-identifying), coerce, and pick by distinct-pages → count → first-seen — the `sponsor_fallback` shape. Display-only (after `policy.adjudicate`), so it earns extraction but can never arm/disarm staleness (CFA-safe). It also removed the pipeline's only nondeterminism (these empty-date cases were rescued or not by an order-dependent path — a "hidden tie" between the valid value and the sentinel; a deterministic recovery pins it). Staleness: `< STALE_CUTOFF (2026-01-02)` & non-DIP → deny.
...
- **Residue:** the value-first fallback fires only when the merge left the field empty, so it does **not** touch the 6/8 *month/primary-read* class (MIB-000381: primary read `08-21` beats a variant plurality `06-21`, truth `06-21`) — a merge/vote question, not a fill one.
